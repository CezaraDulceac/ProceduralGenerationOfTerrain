...
namespace CMU462
{
   ...
   */
   ...
   inline bool operator<( const     EdgeIter& i, const     EdgeIter& j ) { return &*i < &*j; }
   ...
   class Vertex : public HalfedgeElement
   {
      public:
         ...
         glm::vec3 newPosition; ///< For Loop subdivision, this will be the updated position of the vertex
         ...
         glm::vec3 centroid; ///< average of neighbor positions, storing the value computed by Vertex::computeCentroid()
   ...
   class Edge : public HalfedgeElement
   {
      public:
         ...
         {
            glm::vec3 p0 = halfedge()->vertex()->position;
            glm::vec3 p1 = halfedge()->twin()->vertex()->position;
			glm::vec3 aux = (p1 - p0);
			double normE = std::sqrt(aux.x * aux.x + aux.y * aux.y + aux.z * aux.z);
            return normE;
         }

         glm::vec3 newPosition; ///< For Loop subdivision, this will be the position for the edge midpoint
   ...
   class HalfedgeMesh
   {
      public:
          ...
          */
         void build( const vector< vector<Index> >& polygons, const vector<glm::vec3>& vertexPositions );
          ...
          */
		  ...
		  //--------------------------------------------------------------------------
		  //------------------------MyFunctions---------------------------------------
		 VertexCIter findVertexInMesh(glm::vec3 vertex)
		 {
			 for (VertexIter v = this->verticesBegin(); v != this->verticesEnd(); v++)
			 {
				 if (v->position.x == vertex.x && v->position.y == vertex.y && v->position.z == vertex.z)
				 {
					 return v;
				 }
			 }
		 ...
		 VertexCIter findVertexInMeshV(VertexCIter vertex)
		 {
			 for (VertexIter v = this->verticesBegin(); v != this->verticesEnd(); v++)
			 {
				 if (v->position.x == vertex->position.x && v->position.y == vertex->position.x && v->position.z == vertex->position.x)
				 {
					 return v;
				 }
			 }
		 ...
		 void printVertexNeighbours(VertexCIter v)
		 {
		 ...
		 //void printVerteces() {
			...
			// }
			// std::cout << " ---------- \n";
			// //for (VertexCIter v = this->verticesBegin(); v != this->verticesEnd(); v++)
			// //{
			//	// printVertexNeighbours(v); // do something interesting here
			//	// std::cout << "end neighbours \n";
			// //}
		 //};

		 std::vector<glm::vec3> neighbours;
		 std::vector<glm::vec3> findVertexNeighbours(VertexCIter v)
		 {
			 HalfedgeCIter h = v->halfedge(); // get one of the outgoing halfedges of the vertex
			 do
			 {
				 ...
				 neighbours.push_back(glm::vec3(v->position.x, v->position.y, v->position.z));
		 ...
		 std::vector<VertexCIter> findVertexNeighboursV(VertexCIter v)
		 {
			 std::vector<VertexCIter> neighb;
			 HalfedgeCIter h = v->halfedge(); // get one of the outgoing halfedges of the vertex
			 do
			 {
				 HalfedgeCIter h_twin = h->twin(); // get the vertex of the current halfedge
				 ...
				 else if ( !(find(neighb.begin(), neighb.end(), v) != neighb.end()) )
				 {
					 neighb.push_back(v);
				 }
		 ...
		 std::vector<glm::vec3> findNeighbours_CMU(glm::vec3 vertex)
		 {
			 VertexCIter v = findVertexInMesh(vertex);
			 return findVertexNeighbours(v);
		 }

		 vector<vector<VertexCIter>> vecini;
		 std::vector<VertexCIter> aux;
		 vector<vector<VertexCIter>> findNeighbours_CMU_size(glm::vec3 vertex, int size)
		 {
			 
			 VertexCIter v = findVertexInMesh(vertex);
			 findAllNeighbours(v, size);
			 return vecini;
			
		 }
		 
		 vector<VertexCIter> findAllNeighbours(VertexCIter vertex, int size)
		 {
			 if (size)
			 {
				 aux = findVertexNeighboursV(vertex);
				 vecini.push_back(aux);
				 for (auto a : aux)
				 {
					 findAllNeighbours(a, size--);
				 }
				 
			 }
		 }
		 std::vector<glm::vec3> vecini1;
		 void dfs(int size, VertexCIter vertex, VertexCIter parent)
		 {
			 vecini1.push_back(glm::vec3(vertex->position.x, vertex->position.y, vertex->position.z));
			 if (size < 0)
			 {
				 return;
			 }else
			 {
				 ...
				 vector<VertexCIter> n = findVertexNeighboursV(vertex);
				 for (auto i : n)
				 {
					  //std::cout << "vecini: " << i->position.x << " " << i->position.y << " " << i->position.z << "\n";
					 // std::cout << "parent: " <<  parent->position.x << " " << parent->position.y << " " << parent->position.z << "\n";

					 if (!(i->position.x == parent->position.x  && i->position.y == parent->position.y  && i->position.z == parent->position.z))
					 {
						 //std::cout << "AAAAAA\n";
						 size = size-1;
						 dfs(size, i, vertex);
		 ...
		 std::vector<glm::vec3> dfsV(glm::vec3 vertex, int size)
		 {
			 VertexCIter v = findVertexInMesh(vertex);
			 //VertexCIter a;
			 vecini1.clear();
